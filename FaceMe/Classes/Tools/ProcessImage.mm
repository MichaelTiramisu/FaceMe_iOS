...
- (instancetype)init {
    if (self = [super init]) {
        ...
        path = [[NSBundle mainBundle] pathForResource:@"pikachu_ear_left.png" ofType:nil];
        pikachuEarLeft = imread(path.UTF8String, -1);
        
        path = [[NSBundle mainBundle] pathForResource:@"pikachu_ear_right.png" ofType:nil];
        pikachuEarRight = imread(path.UTF8String, -1);
        
        path = [[NSBundle mainBundle] pathForResource:@"pikachu_face_left.png" ofType:nil];
        pikachuFaceLeft = imread(path.UTF8String, -1);
        
        path = [[NSBundle mainBundle] pathForResource:@"pikachu_face_right.png" ofType:nil];
        pikachuFaceRight = imread(path.UTF8String, -1);
    }
    return self;
}
...
- (void)addPikachu:(Mat &)img {
    ...
    for (int i = 0; i < faces.size(); i++)
    {
        ...
        faceRect.height *= scale;
        // 添加左耳朵
        Mat srcImage = pikachuEarLeft.clone();
        double widthScaleFactor = 0.000998;
        double heightScaleFactor = 0.001006;
        int oldWidth = srcImage.cols;
        int oldHeight = srcImage.rows;
        int newWidth = oldWidth * widthScaleFactor * faceRect.width;
        int newHeight = oldHeight * heightScaleFactor * faceRect.height;
        resize(srcImage, srcImage, cv::Size(newWidth, newHeight));
        double xPositionFactor = -0.020906;
        int x = faceRect.x + faceRect.width * xPositionFactor;
        int yOffset = -370;
        int y = faceRect.y + widthScaleFactor * faceRect.width * yOffset;
        cv::Point point = cv::Point(x, y);
        [self addImage:srcImage to:img at:point];
        // 添加右耳朵
        srcImage = pikachuEarRight.clone();
        widthScaleFactor = 0.001009;
        heightScaleFactor = 0.001002;
        oldWidth = srcImage.cols;
        oldHeight = srcImage.rows;
        newWidth = oldWidth * widthScaleFactor * faceRect.width;
        newHeight = oldHeight * heightScaleFactor * faceRect.height;
        resize(srcImage, srcImage, cv::Size(newWidth, newHeight));
        xPositionFactor = 0.68177;
        x = faceRect.x + faceRect.width * xPositionFactor;
        yOffset = -310;
        y = faceRect.y + widthScaleFactor * faceRect.width * yOffset;
        point = cv::Point(x, y);
        [self addImage:srcImage to:img at:point];
        // 添加左脸
        srcImage = pikachuFaceLeft.clone();
        widthScaleFactor = 0.000991;
        heightScaleFactor = 0.001008;
        oldWidth = srcImage.cols;
        oldHeight = srcImage.rows;
        newWidth = oldWidth * widthScaleFactor * faceRect.width;
        newHeight = oldHeight * heightScaleFactor * faceRect.height;
        resize(srcImage, srcImage, cv::Size(newWidth, newHeight));
        xPositionFactor = 0.16260;
        x = faceRect.x + faceRect.width * xPositionFactor;
        double yPositionFactor = 0.54007;
        y = faceRect.y + faceRect.height * yPositionFactor;
        point = cv::Point(x, y);
        [self addImage:srcImage to:img at:point];
        // 添加右脸
        srcImage = pikachuFaceRight.clone();
        widthScaleFactor = 0.00099319;
        heightScaleFactor = 0.000978025;
        oldWidth = srcImage.cols;
        oldHeight = srcImage.rows;
        newWidth = oldWidth * widthScaleFactor * faceRect.width;
        newHeight = oldHeight * heightScaleFactor * faceRect.height;
        resize(srcImage, srcImage, cv::Size(newWidth, newHeight));
        xPositionFactor = 0.63298;
        x = faceRect.x + faceRect.width * xPositionFactor;
        yPositionFactor = 0.55865;
        y = faceRect.y + faceRect.height * yPositionFactor;
        point = cv::Point(x, y);
        [self addImage:srcImage to:img at:point];
    }
